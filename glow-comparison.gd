...
func _ready() -> void:
	...
	for level in range(7):
		...
		_save_image("%s/4.5_foward-hdr2d" % folder_path)

		var table_line: String = ""
		table_line += " | ![mobile-hdr2d]"
		table_line += "(https://github.com/allenwp/godot-glow-comparison/raw/refs/heads/main/%s/mobile-hdr2d.webp)" % [folder_path]
		...
		table_line += " | ![4.5_foward-hdr2d]"
		table_line += "(https://github.com/allenwp/godot-glow-comparison/raw/refs/heads/main/%s/4.5_foward-hdr2d.webp)" % [folder_path]
		print(table_line)
